...
require('mongodb').MongoClient.connect(process.env.mongoURL, function(err, db) {

	var saveEditors = function() {
		//fs.writeFileSync(__dirname + 'editors.json', JSON.stringify(editors));

		db.collection('editors').updateOne({
			name: 'editors',
		}, {
			$set: {
				list: editors
	...
	var getEditors = function() {
		// if (fs.existsSync(__dirname + 'editors.json')) editors = JSON.parse(fs.readFileSync(__dirname + 'editors.json'));
		// else editors = {};

		db.collection('editors').find({
			name: 'editors'
		}).each(function(err, doc) {
			if (doc != null) editors = doc.list;
			else editors = {};
			console.log(editors);
		});
	};
